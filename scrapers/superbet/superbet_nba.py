"""Superbet integration for NBA with ESPN player mapping."""
...
class SuperbetNBA:
    """Superbet NBA data fetcher with ESPN integration.
    
    Fetches NBA odds from Superbet and maps players to ESPN IDs
    for cross-referencing with ESPN stats.
    """
    ...
    async def get_player_props(self, days_ahead: int = 1) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of player prop dictionaries with ESPN mapping
        """
        ...
        for match in matches:
            for market in match.markets:
                ...
                if self._is_player_prop_market(market):
                    ...
                    if prop_data:
                        # Try to map player to ESPN
                        espn_id = self._map_player_to_espn(prop_data['player_name'])
                        prop_data['espn_player_id'] = espn_id
    ...
    def _map_player_to_espn(self, player_name: str) -> Optional[str]:
        """Map player name to ESPN ID using player registry."""
        try:
            ...
            if player:
                log.debug(f"Mapped '{player_name}' to ESPN ID: {player.get('espn_id')}")
                return player.get('espn_id')
            
            log.debug(f"No ESPN mapping found for player: {player_name}")
            return None
