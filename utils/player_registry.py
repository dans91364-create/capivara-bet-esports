...
class PlayerRegistry:
    ...
    def add_player(self, name: str, espn_id: str, sport: str, **metadata):
        ...
        if normalized_name not in self.registry:
            ...
            self.save_registry()
            log.info(f"Added player {name} ({espn_id}) to registry")
        else:
    ...
    def get_espn_id(self, name: str) -> Optional[str]:
        ...
        Returns:
            ESPN player ID or None
        """
        player = self.get_player(name)
        return player.get("espn_id") if player else None
